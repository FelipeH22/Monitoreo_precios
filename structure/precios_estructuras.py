...
t_0=time.time()
print(f"El precio mínimo registrado entre todos los productos es: {str(heap_producto1.ExtractMin())[1:-3]} de {len(precios)} precios")
print('tiempo usando heap:',time.time()-t_0)
t_0_1=time.time()
menor=99999999
for pr in precios:
    if int(str(pr)[1:-3])<menor: menor=int(str(pr)[1:-3])
print(f"El precio mínimo registrado entre todos los productos es: {menor} de {len(precios)} precios")
